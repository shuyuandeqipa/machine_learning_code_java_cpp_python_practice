...
public class LogisticRegressionNewtonMethod {

    INDArray X;
    INDArray y;// +-1
    int max_iters;//在这里只需要一次迭代就足够了

    public LogisticRegressionNewtonMethod(INDArray x, INDArray y, int max_iters) {
        X = x;
        this.y = y;
        this.max_iters = max_iters;
    }
    public INDArray logisticRegressionWithNewtonMethod(){
        int m=this.X.rows();
        int n=this.X.columns();
        INDArray theta= Nd4j.zeros(n,1);

        for(int i=0;i<max_iters;++i){
            INDArray margins=y.mul((X.mmul(theta)));
//            System.out.println(margins.size(0)+" "+margins.size(1));
            INDArray probs=Nd4j.ones(margins.length()).transpose().div(Transforms.exp(margins).add(1.0));
//            System.out.println(probs.size(0)+" "+probs.size(1));
            INDArray grad=X.transpose().mmul((probs.mul(y))).mul(1.0/m);
            INDArray H=(X.transpose().mmul(Nd4j.diag(probs.mul(Nd4j.ones(probs.length()).transpose().sub(probs)))).mmul(X)).mul(1.0/m);
            System.out.println("H="+H.toString());
            //if the matrix is not singular ,then do the code
            // 一直会出现矩阵奇异的状况
            // 一直会出现矩阵奇异的状况，怎么办？
            theta=theta.sub(InvertMatrix.invert(H,false).mmul(grad));
            System.out.println("theta="+theta);
//            try {
//                theta=theta.sub(InvertMatrix.invert(H,false).mmul(grad));
//            }catch (SingularMatrixException e){
//                System.out.println("矩阵是个奇异矩阵，不可逆！");
//                e.printStackTrace();
//            }
    ...
    public static void main(String[]args){//一直出现数据不匹配的状况
        ...
        INDArray data =a.getColumns(0,1,2);
//        data.getColumn(1).muli(0).addi(a);
        INDArray label =a.getColumn(3);label.muli(2).subi(1);
        System.out.println(data);
        System.out.println(label);
        final int iterations = 1;
        //逻辑斯谛回归是用来分类使用的
        LogisticRegressionNewtonMethod logisticRegressionNewtonMethod=new LogisticRegressionNewtonMethod(data,label,iterations);
        INDArray theta=logisticRegressionNewtonMethod.logisticRegressionWithNewtonMethod();
        System.out.println(theta);
        System.out.println(label);
        INDArray predict=data.mmul(theta);
        System.out.println(predict);
        INDArray judge=predict.mul(label);
        int right=0;
        for(int i=0;i<judge.length();i++){
            if(judge.getDouble(i)<0){
                right++;
            }
        }
        System.out.println("accuracy="+(double)right/judge.length()*100+"%");
...
/**
 反思：1.y的数据类型要注意
 2.newton梯度下降法使用的前提条件，到底要进行几次的迭代就足够了
 3.方法是分类任务还是回归任务
